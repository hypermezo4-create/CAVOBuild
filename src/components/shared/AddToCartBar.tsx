...
export function AddToCartBar({ product }: { product: Product }) {
  ...
  return (
    <div className="add-to-cart-bar">
      <div className="size-pills size-pills-scroll">
      ...
      <button type="button" className="primary-button full" onClick={() => addToCart(product.id, size)}>
        أضف للسلة — {product.price.toLocaleString()} EGP
      </button>
    </div>
  );
}
